...
module ScreenToLogic =
  ...
  let ``screen center maps to camera position``() =
    ...
    Assert.Equal(cameraPos.X, result.X, 0.001)
    Assert.Equal(cameraPos.Y, result.Y, 0.001)
  ...
  let ``offset from center is scaled by zoom``() =
    ...
    Assert.Equal(50.0f, result.X, 0.001) // 100 / 2 = 50
...
module Properties =
  ...
  /// Generator for reasonable logic positions (avoids extreme values)
  let logicPosGen =
    Gen.map2
      (fun x y -> Vector2(float32 x, float32 y))
      (Gen.choose(-10000, 10000))
      (Gen.choose(-10000, 10000))
    |> Arb.fromGen

  /// Generator for positive PPU values
  let ppuGen =
    Gen.map2
      (fun x y -> Vector2(float32 x, float32 y))
      (Gen.choose(1, 128))
  ...
  let ``LogicToRender X component is always logicX / ppu.X``() =
    Prop.forAll logicPosGen (fun logicPos ->
      Prop.forAll ppuGen (fun ppu ->
  ...
  [<Property>]
  let ``GetSquishFactor is always ppu.X / ppu.Y``() =
    Prop.forAll ppuGen (fun ppu ->
