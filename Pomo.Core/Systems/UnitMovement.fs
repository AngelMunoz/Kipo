...
module UnitMovement =
  ...
  type UnitMovementSystem(game: Game, playerId: Guid<EntityId>) =
    ...
    override this.Update(gameTime) =
      let movementStates = this.World.MovementStates |> AMap.force
      let positions = this.World.Positions |> AMap.force
      ...
      for entityId, state in movementStates do
        // Skip player as PlayerMovementSystem handles it
