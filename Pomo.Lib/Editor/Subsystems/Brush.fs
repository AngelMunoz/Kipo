...
module Brush =
  ...
  let update (msg: Msg) (model: BrushModel) : struct (BrushModel * Cmd<Msg>) =
    ...
    | RotateY deg ->
        ...
        { model with Rotation = rot * model.Rotation }, Cmd.none
    | ToggleCollision -> { model with CollisionEnabled = not model.CollisionEnabled }, Cmd.none
    | SetDragging(dragging, cell) -> { model with IsDragging = dragging; DragStartCell = if dragging then cell else ValueNone }, Cmd.none
    | SetLastPlacedCell cell -> { model with LastPlacedCell = cell }, Cmd.none
    | ResetRotation -> { model with Rotation = Quaternion.Identity }, Cmd.none

  let view ctx (model: BrushModel) (cursorCell: Vector3 voption) (buffer: RenderBuffer<unit, RenderCommand>) =
